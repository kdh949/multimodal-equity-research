...
def test_validity_gate_passes_structural_contract_for_clean_inputs() -> None:
    ...
    assert report.metrics["target_horizon_periods"] == 5
...
def test_validity_gate_hard_fails_horizon_embargo_violation() -> None:
    ...
    assert any("gap_periods=1" in reason for reason in report.hard_fail_reasons)
    assert any("embargo_periods=1" in reason for reason in report.hard_fail_reasons)
    assert report.metrics["purge_embargo_application"]["status"] == "hard_fail"
